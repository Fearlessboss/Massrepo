...
║   ✅ POWERFUL report messages (multi-paragraph, contextual)          ║
║   ✅ /logs sends actual log FILE (not just text)                     ║
...
║   🆕 /report → FAST batch mode, ONE summary message (no spam)        ║
║   🆕 /groupreport → Group 3-dot menu flow (report on specific msg)   ║
╚══════════════════════════════════════════════════════════════════════╝
"""
...
# ══════════════════════════════════════════════════════════════════════
# 📋 FULL TELEGRAM REPORT CATEGORY TREE
# ══════════════════════════════════════════════════════════════════════
...
def craft_report_message(base_msg: str, sub_label: str = "") -> str:
    ...
    final = " ".join(parts)
    # Telegram report message limit is around 512 chars — keep it safe
    if len(final) > 480:
        final = final[:477] + "..."
    return final
...
async def ensure_connected(phone: str) -> Optional[TelegramClient]:
    """
    Critical fix for 'Cannot send requests while disconnected':
    - Always check connection before using a client
    - Reconnect if disconnected
    - Returns a usable, authorized client or None
    """
    ...
    try:
        if not client.is_connected():
            ...
            if not ok:
                return None
            client = accounts.get(phone)  # may have been swapped to direct
        # quick auth check
        try:
...
def looks_like_session_string(text: str) -> bool:
    """
    Detect if input is a session string (Telethon or Pyrogram) vs phone number.
    - Phone: starts with + or digits, length 8-16
    - Session string: typically 250+ chars, base64-like
    """
    t = text.strip()
    if not t:
        return False
    ...
    if cleaned.isdigit() and len(cleaned) <= 16:
        return False
    # Session strings are long base64-ish
...
def convert_pyrogram_to_telethon(pyro_string: str) -> Optional[str]:
    ...
    Pyrogram sessions encode: dc_id, api_id, test_mode, auth_key, user_id, is_bot
    Telethon needs: dc_id, server_address, port, auth_key
    """
    try:
        import base64
        import struct
        ...
        # Pyrogram v2 session string format:
        # base64(struct: >BI?256sQ?  =  dc_id, api_id, test_mode, auth_key(256), user_id, is_bot)
        # Pyrogram v1 (older): >B?256sI?  = dc_id, test_mode, auth_key, user_id, is_bot
        ...
        # Try v2 format (most common today)
        ...
        # Try v1 format
            ...
            auth_key = auth_key_bytes
        # Try v3-ish (with longer user id)
...
async def try_load_session_string(session_str: str) -> Tuple[Optional[TelegramClient], Optional[str], str]:
    """
    Try loading the session string as Telethon first, then as Pyrogram.
    Returns: (client, phone_or_id, error_msg)
    """
    proxy = next_proxy()
...
async def send_report(phone, channel_id, msg_id, reason_api, custom_msg,
    ...
    # 🔥 KEY FIX: ensure client is connected before EVERY report attempt
    client = await ensure_connected(phone)
    if not client:
...
# ══════════════════════════════════════════════════════════════════════
# 🆕 GROUP REPORT ENGINE — Simulates 3-dot menu flow on a specific message
# ══════════════════════════════════════════════════════════════════════
async def send_group_report(phone, channel_id, msg_id, reason_api, custom_msg,
                            link_type="username", sub_label="") -> Tuple[bool, str]:
    """
    Simulates the in-group 3-dot menu report flow:
    1. Open group/channel (resolve entity)
    2. Click 3-dot → Report (select reason)
    3. Paste optional reason message
    4. Submit report → it auto-targets the specific message given by msg_link

    Uses the same Telegram API (messages.ReportRequest) under the hood,
    because that's exactly what the in-app 3-dot menu calls.
    """
    client = await ensure_connected(phone)
    if not client:
        return (False, "Account disconnected / unauthorized")
    ...
    proxy = account_proxy_map.get(phone)
    try:
        # Step 1: "Open" the group — resolve peer entity
        try:
            entity = await resolve_chat_entity(client, channel_id, link_type)
        except Exception as e:
            mark_proxy_result(proxy, False)
            return (False, f"Open group fail: {str(e)[:50]}")

        msg_id_int = int(msg_id)
        await asyncio.sleep(random.uniform(0.2, 0.5))  # simulate human delay

        # Step 2: Prefetch message (simulate scrolling to it before tapping 3-dot)
        try:
            client = await ensure_connected(phone)
            if not client:
                return (False, "Disconnected before report")
        ...
        # Step 3 + 4: Tap 3-dot → Report → submit with selected reason + optional message
        crafted_msg = craft_report_message(custom_msg, sub_label)
        try:
            result = await client(functions.messages.ReportRequest(
                peer=entity,
                id=[msg_id_int],         # targets the specific message from msg_link
                reason=reason_api,
                message=crafted_msg
            ))
            if result:
                mark_proxy_result(proxy, True)
                add_log(f"✅ GRP-M1 OK: {phone[-4:]}")
                return (True, "Success (GRP-M1: 3-dot report)")
        except errors.FloodWaitError as e:
            return (False, f"FloodWait {e.seconds}s")
        except errors.MessageIdInvalidError:
            return (False, "Invalid Message ID")
            ...
            add_log(f"⚠️ GRP-M1 fail {phone[-4:]}: {type(e).__name__}")

        # Fallback M2: re-resolve + retry
        ...
        try:
            ...
            result = await client(functions.messages.ReportRequest(
                peer=entity, id=[msg_id_int],
                reason=reason_api, message=crafted_msg))
            if result:
                mark_proxy_result(proxy, True)
                add_log(f"✅ GRP-M2 OK: {phone[-4:]}")
                return (True, "Success (GRP-M2: retry)")
        except errors.FloodWaitError as e:
            return (False, f"FloodWait {e.seconds}s")
        except Exception as e:
            add_log(f"⚠️ GRP-M2 fail {phone[-4:]}: {type(e).__name__}")

        # Fallback M3: account.reportPeer (whole chat)
        await asyncio.sleep(random.uniform(0.3, 0.6))
        try:
            client = await ensure_connected(phone)
            if not client:
            ...
            result = await client(functions.account.ReportPeerRequest(
                peer=entity, reason=reason_api,
                message=f"Targeted msg ID {msg_id}. {crafted_msg}"))
            if result:
                mark_proxy_result(proxy, True)
                add_log(f"✅ GRP-M3 OK: {phone[-4:]}")
                return (True, "Success (GRP-M3: reportPeer)")
        except Exception as e:
            add_log(f"⚠️ GRP-M3 fail {phone[-4:]}: {type(e).__name__}")
...
def smart_delay(i, total) -> float:
    ...
    extra = random.uniform(0, 6) if random.random() < 0.4 else 0
...
# 🆕 Fast delays for batch mode
def fast_round_delay() -> float:
    return random.uniform(0.8, 1.8)

def fast_switch_delay() -> float:
...
async def menu_router(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    info_map = {
        "report":        ("📩 <b>Message Report</b>\n\nUse /report — fast batch mode, ONE summary message.", "/report"),
        "groupreport":   ("👥 <b>Group Report</b>\n\nUse /groupreport — mimics in-app 3-dot menu flow.", "/groupreport"),
...
async def help_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        "🎯 /report — Message report (fast batch, ONE summary)\n"
        "👥 /groupreport — Group 3-dot menu flow on specific msg\n"
...
async def logs_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Send actual log files instead of just text."""
    if not is_authorized(update.effective_user.id): return
...
async def _do_restart():
    """Save state, disconnect all clients cleanly, then re-exec the process."""
    try:
        save_accounts()
        save_sudo()
        save_proxy_health()
...
# ══════════════════════════════════════════════════════════════════════
# 📱 ADD ACCOUNT — supports PHONE+OTP and SESSION STRING
# ══════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════
# 🎯 MESSAGE REPORT FLOW — 🆕 FAST BATCH MODE, ONE SUMMARY MESSAGE
# ══════════════════════════════════════════════════════════════════════
async def report_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        ...
        f"⚡ <b>FAST BATCH MODE</b> — ONE summary message at end\n\n"
...
async def report_execute(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    """🆕 FAST BATCH MODE: All reports fire in parallel-ish, ONE summary message."""
    try:
    ...
    # Build authorized pairs with reconnect
    auth_pairs = []
    for phone in list(accounts.keys()):
        c = await ensure_connected(phone)
        if c:
    ...
    progress_msg = await update.message.reply_text(
        f"🚀 <b>FAST BATCH REPORT STARTED</b>\n\n"
        f"📊 Total: {total_planned}\n"
        f"📱 Accounts: {len(auth_pairs)} × {count}\n"
        f"⚠️ {cat_lbl} → {sub_lbl}\n"
        f"⚡ Mode: FAST batch (parallel per round)\n"
        f"🌐 {join_note}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"⏳ Firing... please wait, ONE summary at end.",
        parse_mode="HTML")

    total_ok = total_fail = 0
    per_acc_ok: Dict[str, int]   = {p: 0 for p, _ in auth_pairs}
    per_acc_fail: Dict[str, int] = {p: 0 for p, _ in auth_pairs}
    per_acc_last_status: Dict[str, str] = {p: "" for p, _ in auth_pairs}
    floodwait_warnings: List[str] = []

    # 🚀 FAST MODE: Fire all accounts in parallel per round
    for r in range(count):
        # Build tasks for all accounts in this round
        tasks = []
        for phone, _ in auth_pairs:
            tasks.append(send_report(
                phone, ident, msg_id, reason_api, custom_msg,
                link_type, sub_lbl))
        ...
        # Tally results
        for (phone, _), result in zip(auth_pairs, results):
            if isinstance(result, Exception):
                total_fail += 1
                per_acc_fail[phone] += 1
                per_acc_last_status[phone] = f"Exception: {type(result).__name__}"
                update_stats(False)
                log_report_file(phone, msg_id, f"{cat_lbl}/{sub_lbl}", "FAILED", str(result)[:50])
            else:
                ok, status = result
                if ok:
                    total_ok += 1
                    per_acc_ok[phone] += 1
                    per_acc_last_status[phone] = status
                    update_stats(True)
                    ...
                    if "FloodWait" in status:
                        floodwait_warnings.append(f"{phone[-4:]}: {status}")

        # Tiny inter-round delay to avoid global flood
        if r < count - 1:
            await asyncio.sleep(fast_switch_delay())

        # Update progress every few rounds (light edit, no spam)
        if (r + 1) % max(1, count // 3) == 0 or r == count - 1:
            try:
                done = (r + 1) * len(auth_pairs)
                await progress_msg.edit_text(
                    f"🚀 <b>FAST BATCH REPORT</b>\n\n"
                    f"📊 Progress: {done}/{total_planned}\n"
                    f"✅ {total_ok} | ❌ {total_fail}\n"
                    f"🔁 Round {r+1}/{count} done\n"
                    f"━━━━━━━━━━━━━━━━━━━━\n"
                    f"⏳ Continuing...",
                    parse_mode="HTML")
            except: pass
    ...
    fw_section = ""
    if floodwait_warnings:
        fw_section = f"\n\n⚠️ <b>FloodWaits hit:</b>\n" + "\n".join(f"  • {w}" for w in floodwait_warnings[:5])
        if len(floodwait_warnings) > 5:
            fw_section += f"\n  ...and {len(floodwait_warnings)-5} more"

    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("📧 Gmail Blast (next)", callback_data="GMAIL_START")],
        [InlineKeyboardButton("🏠 Done",               callback_data="GMAIL_SKIP")]])

    # 🎯 ONE FINAL SUMMARY MESSAGE
    final_summary = (
        f"━━━━━━━━━━━━━━━━━━━━\n"
        ...
        f"⚡ Speed: {(total_planned/max(elapsed,1)):.1f} reports/sec\n\n"
    ...
    try:
        await progress_msg.edit_text(final_summary, reply_markup=keyboard, parse_mode="HTML")
    except Exception:
        await update.message.reply_text(final_summary, reply_markup=keyboard, parse_mode="HTML")
    return MAIL_SUBJECT

# ══════════════════════════════════════════════════════════════════════
# 🆕 GROUP REPORT FLOW — 3-DOT MENU SIMULATION
# ══════════════════════════════════════════════════════════════════════
async def groupreport_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        f"👥 <b>GROUP REPORT (3-DOT MENU FLOW) — Step 1/7</b>\n\n"
        f"✅ Active: {active}/{len(accounts)} accounts\n"
        f"💡 Mimics: Open group → 3-dot → Report → Reason → Msg → Submit\n"
        f"🎯 Submits report targeting the specific message you provide\n\n"
        f"📥 Send <b>GROUP / CHANNEL LINK</b>:\n\n"
        f"  • Public  → <code>t.me/groupname</code>\n"
        f"  • Private → <code>t.me/+invitehash</code>\n\n"
        f"/cancel to abort.",
...
async def gr_receive_grp_link(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    add_log(f"👥 GroupReport target: {link} (type: {ltype})")
    if ltype == "username":
        await update.message.reply_text(
            f"ℹ️ Public group — direct report (no join needed).\n\n"
            f"📥 <b>Step 2/7</b> — Send <b>MESSAGE LINK</b> (the one to report):\n"
            f"  • <code>t.me/groupname/123</code>\n"
            f"  • <code>t.me/c/1234567890/123</code>",
            parse_mode="HTML")
    else:
        ...
        await update.message.reply_text(
            f"✅ Join complete!\n✅ {ok} | ❌ {fail}\n\n"
            f"📥 <b>Step 2/7</b> — Send <b>MESSAGE LINK</b> (the one to report):", parse_mode="HTML")
    return GR_MSG_LINK
...
async def gr_receive_msg_link(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        "📋 <b>Step 3/7</b> — Tap a reason (like the 3-dot menu options):",
        reply_markup=_build_full_cat_keyboard("GCAT"),
        parse_mode="HTML")
...
async def gr_category_selected(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if cat["direct"]:
        ...
        await query.edit_message_text(
            f"✅ {cat['emoji']} {cat['label']}\n\n"
            f"📝 <b>Step 5/7</b> — Optional reason text or send <code>skip</code>:",
            parse_mode="HTML")
    ...
    await query.edit_message_text(
        f"✅ {cat['emoji']} {cat['label']}\n\n"
        f"📋 <b>Step 4/7</b> — Select sub-category:",
        reply_markup=_build_sub_keyboard(cat_key, "GSUB"),
        parse_mode="HTML")
...
async def gr_execute(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    """🆕 GROUP REPORT: 3-dot menu simulation, FAST batch mode, ONE summary."""
    try:
        count = int(update.message.text.strip())
        if not 1 <= count <= MAX_REPORTS_PER_ACCOUNT:
            await update.message.reply_text(f"⚠️ Enter 1–{MAX_REPORTS_PER_ACCOUNT}:")
    ...
    ident      = ctx.user_data["gr_msg_ident"]
    ...
    grp_link   = ctx.user_data.get("gr_grp_link", "")
    ...
    progress_msg = await update.message.reply_text(
        f"🚀 <b>GROUP REPORT (3-DOT FLOW) STARTED</b>\n\n"
        f"👥 Group: <code>{grp_link}</code>\n"
        f"🎯 Target msg: <code>{msg_link}</code>\n"
        f"📊 Total: {total_planned}\n"
        f"📱 Accounts: {len(auth_pairs)} × {count}\n"
        f"⚠️ {cat_lbl} → {sub_lbl}\n"
        f"⚡ Mode: FAST batch + 3-dot menu sim\n"
        f"🌐 {join_note}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"⏳ Firing... please wait, ONE summary at end.",
        parse_mode="HTML")
    ...
    for r in range(count):
        tasks = []
        for phone, _ in auth_pairs:
            tasks.append(send_group_report(
                phone, ident, msg_id, reason_api, custom_msg,
                link_type, sub_lbl))
        ...
        for (phone, _), result in zip(auth_pairs, results):
            if isinstance(result, Exception):
                ...
                per_acc_fail[phone] += 1
                update_stats(False)
                log_report_file(phone, msg_id, f"GRP-{cat_lbl}/{sub_lbl}", "FAILED", str(result)[:50])
            else:
                ok, status = result
                if ok:
                    total_ok += 1
                    per_acc_ok[phone] += 1
                    update_stats(True)
                    log_report_file(phone, msg_id, f"GRP-{cat_lbl}/{sub_lbl}", "SUCCESS", status)
                else:
                    total_fail += 1
                    per_acc_fail[phone] += 1
                    update_stats(False)
                    log_report_file(phone, msg_id, f"GRP-{cat_lbl}/{sub_lbl}", "FAILED", status)
                    if "FloodWait" in status:
                        floodwait_warnings.append(f"{phone[-4:]}: {status}")
    ...
    final_summary = (
        ...
        f"<b>Per-account breakdown:</b>\n{breakdown}\n\n"
        f"👥 Group: {grp_link}\n"
        f"🎯 Target msg: {msg_link}\n"
        f"⚠️ {cat_lbl} → {sub_lbl}"
        f"{fw_section}\n\n"
        f"Use /groupreport for another target.")

    try:
        await progress_msg.edit_text(final_summary, parse_mode="HTML")
    except Exception:
...
def main():
    ...
    gr_conv = ConversationHandler(
        ...
        states={
            ...
            GR_REASON_CAT: [CallbackQueryHandler(gr_category_selected, pattern="^GCAT\\|")],
            GR_REASON_SUB: [CallbackQueryHandler(gr_subcategory_selected, pattern="^GSUB\\|")],
    ...
    app.add_handler(gr_conv)           # 🆕 group report
