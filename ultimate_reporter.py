"""
╔══════════════════════════════════════════════════════════════════════╗
║   ⚡ ULTIMATE TELEGRAM REPORTER v14.0 — CYBER JUSTICE ELITE+ ⚡      ║
║──────────────────────────────────────────────────────────────────────║
...
async def send_report(phone, channel_id, msg_id, reason_api, custom_msg,
    ...
    try:
        ...
        try:
            ...
            # disconnect happened — reconnect and retry once
...
async def help_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        "🎯 /report — Report a message (full TG categories)\n"
...
async def logs_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # First send stats + last 30 lines as text preview
    txt = f"📊 <b>ACTIVITY STATS</b>\n\n{get_stats()}\n\n<b>Recent logs:</b>\n<pre>{get_logs(30)}</pre>"
    if len(txt) > 4000: txt = txt[:3990] + "\n...(truncated)"
    await update.message.reply_text(txt, parse_mode="HTML")

    # Send today's activity log file
...
async def _do_restart():
    ...
    # Re-exec the python process — sessions are persisted in accounts.json
...
async def add_phone(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # 🔥 KEY FEATURE: auto-detect session string vs phone
...
async def report_execute(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        f"🚀 <b>REPORTING STARTED (ROUND-ROBIN) — Step 7/7</b>\n\n"
    ...
    for r in range(count):
        for acc_idx, (phone, _) in enumerate(auth_pairs):
            ...
            if ok:
                ...
                try:
                    await update.message.reply_text(
                        f"✅ {shot_num}/{total_planned} | R{r+1} | 📱 {phone[-4:]} → {status}",
                        parse_mode="HTML")
                except: pass
            else:
...
async def post_init(application):
    ...
    load_proxy_health()
    load_accounts()
    # Connect all loaded accounts on startup
    for phone in list(accounts.keys()):
        try:
